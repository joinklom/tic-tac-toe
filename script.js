...
buttonSubmit.addEventListener("click", () => {
    const inputName1 = document.querySelector("#playerOne");
    buttonSubmit.style.display = "none";
    chosenName = document.querySelector("#playerOne").value;
    inputName1.style.display = "none";
    playerOne.name = chosenName;
    return;
}) 

const playerOne = player("Player","X",1,[],0);
const playerTwo = player("Opponent","O",2,[],0);
...
function flicker(element){
    ...
    const firstInterval  = setInterval(flickerOff, 500)
    const secondInterval = setInterval(flickerOn, 1000)
    if (victory === false) {
